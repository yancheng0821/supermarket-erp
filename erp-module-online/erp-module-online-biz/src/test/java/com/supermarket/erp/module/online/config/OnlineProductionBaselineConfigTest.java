package com.supermarket.erp.module.online.config;
...
class OnlineProductionBaselineConfigTest {

    private final YamlPropertySourceLoader yamlLoader = new YamlPropertySourceLoader();

    @Test
    void onlineProfileFiles_shouldExposeProductionBaseline() throws IOException {
        ...
        assertThat(base.getProperty("spring.application.name")).isEqualTo("erp-online");
        ...
        assertThat(base.getProperty("logging.file.path")).isEqualTo("${ERP_ONLINE_LOG_PATH:./logs/erp-online}");
        assertThat(base.getProperty("erp.security.permit-all-urls[0]")).isEqualTo("/api/v1/app/online/**");
        assertThat(base.getProperty("erp.security.permit-all-urls[1]")).isEqualTo("/actuator/health");
        assertThat(base.getProperty("erp.security.permit-all-urls[2]")).isEqualTo("/actuator/info");
        assertThat(base.getProperty("erp.security.permit-all-urls[3]")).isNull();
    }

    @Test
    void onlineLocalProfile_shouldDisableNacosAndExternalizeDatasourceSecrets() throws IOException {
    ...
    @Test
    void onlineLogback_shouldSeparateAccessAndAsyncFileAppenders() throws IOException {
        String logback = readTextResource("logback-spring.xml");

        assertThat(logback).contains("<springProperty scope=\"context\" name=\"APP_NAME\" source=\"spring.application.name\" defaultValue=\"erp-online\"/>");
