...
interface StoreSettlement {
  id: number; settlementNo: string; storeName: string; periodStart: string
...
export function StoreSettlementPage() {
  ...
  const [data, setData] = useState<StoreSettlement[]>([])
  const [total, setTotal] = useState(0)
  const [page, setPage] = useState(1)
  const [keyword, setKeyword] = useState('')
  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState<StoreSettlement | null>(null)
  const [form, setForm] = useState({ storeName: '', periodStart: '', periodEnd: '', totalAmount: 0 })

  const fetchData = async () => {
    try {
      const res = await api.get<PageResult<StoreSettlement>>('/admin/finance/store-settlement/page', { page, size: 10, keyword })
  ...
  const handleSave = async () => {
    try {
      if (editing) {
        await api.put(`/admin/finance/store-settlement/${editing.id}`, form)
        toast.success(t('common.operationSuccess'))
      } else {
        await api.post('/admin/finance/store-settlement', form)
        toast.success(t('common.operationSuccess'))
      }
  ...
  const openEdit = (item: StoreSettlement) => {
    setEditing(item)
    setForm({ storeName: item.storeName, periodStart: item.periodStart, periodEnd: item.periodEnd, totalAmount: item.totalAmount })
    setOpen(true)
  }

  const openCreate = () => {
    setEditing(null)
    setForm({ storeName: '', periodStart: '', periodEnd: '', totalAmount: 0 })
    setOpen(true)
  }
    ...
    <>
      ...
      <Main>
        <div className='mb-4 flex items-center justify-between'>
          <h1 className='text-2xl font-bold'>{t('finance.storeSettlement.title')}</h1>
