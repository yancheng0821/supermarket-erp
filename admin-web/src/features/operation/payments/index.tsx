...
interface Payment {
  id: number; paymentNo: string; orderId: number; method: string
...
export function PaymentsPage() {
  ...
  const statusMap: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline' }> = {
    ...
    success: { label: t('operation.payments.success'), variant: 'default' },
    failed: { label: t('operation.payments.failed'), variant: 'destructive' },
    refunded: { label: t('operation.sales.refunded'), variant: 'secondary' },
  }

  const methodLabels: Record<string, string> = {
    cash: t('operation.payments.cash'), wechat: t('operation.payments.wechat'), alipay: t('operation.payments.alipay'), card: t('operation.payments.bankCard'),
  }
  ...
  const fetchData = async () => {
    try {
      const res = await api.get<PageResult<Payment>>('/admin/operation/payment/page', { page, size: 10, keyword })
    ...
    <>
      ...
      <Main>
        <div className='mb-4 flex items-center justify-between'>
          <h1 className='text-2xl font-bold'>{t('operation.payments.title')}</h1>
        </div>
            ...
            <TableBody>
              {data.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Badge variant='outline'>{methodLabels[item.method] || item.method}</Badge>
                  </TableCell>
