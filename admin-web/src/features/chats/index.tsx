...
import { getChatConversations } from './data/conversations'

export function Chats() {
  const { t } = useTranslation()
  const conversations = getChatConversations(t)
  ...
  const dateGroupFormat = t('chats.formats.dateGroup')
  const timeFormat = t('chats.formats.time')

  const currentMessage = selectedUser?.messages.reduce(
    (acc: Record<string, Convo[]>, obj) => {
      const key = format(new Date(obj.timestamp), dateGroupFormat)
          ...
          <div className='flex w-full flex-col gap-2 sm:w-56 lg:w-72 2xl:w-80'>
            <div className='sticky top-0 z-10 -mx-4 bg-background px-4 pb-3 shadow-md sm:static sm:z-auto sm:mx-0 sm:p-0 sm:shadow-none'>
              ...
              >
                ...
                <span className='sr-only'>{t('chats.searchLabel')}</span>
                <input
                  type='text'
            ...
            <ScrollArea className='-mx-3 h-full overflow-scroll p-3'>
              {filteredChatList.map((chatUsr) => {
                ...
                const lastMsg =
                  lastConvo.isSelf
                    ? `${t('chats.youPrefix')}${lastConvo.message}`
                    : lastConvo.message
                return (
                  <Fragment key={id}>
                    <button
              ...
              <div className='mb-1 flex flex-none justify-between bg-card p-4 shadow-lg sm:rounded-t-md'>
                ...
                <div className='-me-1 flex items-center gap-1 lg:gap-2'>
                  ...
                  <Button
                    ...
                    variant='ghost'
                    className='hidden size-8 rounded-full sm:inline-flex lg:size-10'
                    aria-label={t('chats.phoneCall')}
                    title={t('chats.phoneCall')}
                  >
                        ...
                        Object.keys(currentMessage).map((key) => (
                          <Fragment key={key}>
                            {currentMessage[key].map((msg, index) => (
                              <div
                                key={`${msg.senderName}-${msg.timestamp}-${index}`}
                                className={cn(
                              ...
                              >
                                ...
                                >
                                  {format(new Date(msg.timestamp), timeFormat)}
                                </span>
                              </div>
                            ))}
                  ...
                  <div className='flex flex-1 items-center gap-2 rounded-md border border-input bg-card px-2 py-1 focus-within:ring-1 focus-within:ring-ring focus-within:outline-hidden lg:gap-4'>
                    <div className='space-x-1'>
                      <Button
                        ...
                        aria-label={t('chats.addAction')}
                        title={t('chats.addAction')}
                      >
                      ...
                      <Button
                        ...
                        aria-label={t('chats.addImage')}
                        title={t('chats.addImage')}
                      >
                        <ImagePlus
                          size={20}
                    ...
                    <label className='flex-1'>
                      <span className='sr-only'>{t('chats.messageInputLabel')}</span>
                      <input
                        type='text'
            ...
            >
              <div className='flex flex-col items-center space-y-6'>
                ...
                <div className='space-y-2 text-center'>
                  <h1 className='text-xl font-semibold'>{t('chats.emptyTitle')}</h1>
