...
interface StockItem {
  id: number; productId: number; locationType: string; locationId: number
  quantity: number; costAmount: number
}
...
export function StockOverviewPage() {
  ...
  const fetchData = async () => {
    try {
      const res = await api.get<PageResult<StockItem>>('/admin/inventory/stock/page', { page, size: 10, keyword })
    ...
    <>
      ...
      <Main>
        <div className='mb-4 flex items-center justify-between'>
          <h1 className='text-2xl font-bold'>{t('inventory.stock.title')}</h1>
        </div>
